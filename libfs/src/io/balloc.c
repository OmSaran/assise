	...
	unsigned long num_blocks, unsigned long *new_blocknr)
{
	...
	while (temp) {
		...
		if (num_blocks >= curr_blocks) {
			if (num_blocks > curr_blocks) {
				// FIXME: temporarily commenting out panic call.
				// Need to think about implications of going to the next rb tree node when
				// current size is smaller than requested. How does it affect performance?
